...
export class PaymentPage extends PolicyCenterPage {
  ...
  paymentMethodAutomatic = this.page.getByLabel("Automatic", { exact: true });
  paymentMethodManual = this.page.getByLabel("Manual", { exact: true });
  addButton = this.page.getByRole("button", { name: "Add" });
  issuePolicyButton = this.page.getByRole("button", { name: "Issue Policy" });
  mostRecentAccountRadio = this.page
    .locator('div[id*="PaymentMethod_radio"]')
    .last();
